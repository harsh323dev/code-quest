...
const QuestionDetails = () => {
  ...
  const fetchQuestionDetails = async () => {
    try {
      ...
      const found = res.data.find((q: Question) => q._id === id);
      setQuestion(found);
    } catch (error) {
      console.log(error);
    } finally {
  ...
  const handleVote = async (value: string) => {
    ...
    try {
      await axiosInstance.patch(`/question/vote/${id}`, {
        value, 
        userId: user._id
      });
      fetchQuestionDetails(); 
    } catch (error: any) {
      const msg = error.response?.data?.message || "Voting failed";
      toast.error(msg);
    }
  };
                ...
                <div className="flex gap-6">
                    ...
                    <div className="flex flex-col items-center gap-2 w-10 shrink-0">
                        <button 
                          ...
                          className={`border rounded-full w-8 h-8 flex items-center justify-center transition ${hasUpVoted ? 'bg-orange-500 text-white border-orange-500' : 'hover:bg-gray-100 text-gray-600'}`}
                        >
                          ▲
                        </button>
                        ...
                        <button 
                          ...
                          className={`border rounded-full w-8 h-8 flex items-center justify-center transition ${hasDownVoted ? 'bg-orange-500 text-white border-orange-500' : 'hover:bg-gray-100 text-gray-600'}`}
                        >
                          ▼
                        </button>
                    </div>
                    ...
                    <div className="flex-1">
                        ...
                        <div className="space-y-6 mb-8">
                            {question.answer?.map((ans: Answer) => (
                        ...
                        <div className="mt-8">
                            ...
                            <form onSubmit={handlePostAnswer}>
                                ...
                                />
                                <button type="submit" className="bg-blue-600 text-white px-4 py-2 rounded mt-3 hover:bg-blue-700">
                                    Post Answer
                                </button>
                            </form>
                        </div>
